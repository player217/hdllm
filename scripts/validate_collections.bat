@echo off
REM Collection Validation Batch Script for HD현대미포 Gauss-1 RAG System
REM Author: Claude Code
REM Date: 2025-01-26

echo.
echo ========================================
echo  HD현대미포 Gauss-1 컬렉션 검증 스크립트
echo ========================================
echo.
...
echo ========================================
if %EXIT_CODE% == 0 (
    echo  ✅ 검증 완료 - 모든 컬렉션 정상
) else if %EXIT_CODE% == 1 (
    echo  ❌ 검증 실패 - 시스템 설정 확인 필요
) else if %EXIT_CODE% == 2 (
    echo  ⚠️  검증 완료 - 일부 문제 발견
) else (
